...
const { getAllStudents } = require("../redux/StudentActions");
...
describe('Integration Test for StudentActions with middleware Thunk using jest', ()=> {
    ...
    test('getAllStudents', async()=> {
        const studentList = [
            {id:1, sname:'Mayank', course:'java'},
            {id:5, sname:'Rajat', course:'python'},
            {id:6, sname:'Gaurav', course:'ML'}

        ]
        api.get.mockResolvedValue({data:studentList})
    ...
    test('getAllStudents should not update store students[]', async() => {
        ...
        await store.dispatch(getAllStudents())
        const state = store.toHaveBeenCalledTimes(1)
    ...
    test('addStudent', async()=> {
        const student = {
            sname:'Mayank', course:'react'
        }

        const saveStudent = {id:4, sname:'Rajat', course:'spring'}
    ...
    test('getStudentById', async()=> {
        const student = {
           id:4, sname:'Mayank', course:'react'
        }

        const saveStudent = {id:4, sname:'Rajat', course:'spring'}

        api.get.mockResolvedValue({data:saveStudent})
       ...
       test('getStudent should not update student', async() => {
